...
| 类型 | 技术选型 | 用途 |
|------|---------|------|
| **关系型数据库** | PostgreSQL 15+ | 核心业务数据（用户、课程、订单等） |
| **缓存** | Redis 7+ | 会话管理、热点数据缓存、分布式锁 |
| **搜索引擎** | Elasticsearch 8+ | 课程搜索、内容全文检索 |
| **文档数据库** | MongoDB 6+ | 学习日志、用户行为数据 |
| **时序数据库** | TimescaleDB | 学习时长统计、能力变化曲线 |
| **对象存储** | 阿里云OSS / AWS S3 | 视频、图片、文档存储 |
...
| **日志系统** | ELK Stack (Elasticsearch + Logstash + Kibana) | 日志收集分析 |
